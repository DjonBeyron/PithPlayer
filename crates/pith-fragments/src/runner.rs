...
pub fn sanitize(name: &str) -> String {
    let cleaned: String = name
        .chars()
        .map(|c| match c {
            // Вопрос в имени файла Windows не принимает, а реплики им
            // заканчиваются постоянно. Перевёрнутый знак читается так же,
            // но файловой системе не мешает — так их и подписывают вручную.
            '?' => '¿',
            '<' | '>' | ':' | '"' | '/' | '\\' | '|' | '*' => ' ',
            c if (c as u32) < 32 => ' ',
            c => c,
...
mod tests {
    ...
    #[test]
    fn вопрос_становится_перевёрнутым() {
        // Смысл реплики сохраняется, а файловой системе такой знак не мешает.
        assert_eq!(
            sanitize("Are we having an arguments?"),
            "Are we having an arguments¿"
        );
        assert_eq!(
            sanitize("Что делать? Кто виноват?"),
